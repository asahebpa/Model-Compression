...
if __name__ == '__main__':
    ...
    if path.isfile(saveDir+dataset+"state_dict_model.pt") == False:
        ...
        paramorgtot = []
        paramcompresstot = []
        # www = np.asarray(['fc1.weight', 'fc2.weight'])
        # count = 0
        # for name, param in network2.named_parameters():
        #     if www[count]==name:
        #         ptemp = np.asarray(param.detach().cpu())
        #         Decompressedp, paramorg, paramcompress = MatriceformCompress(ptemp, compressdim[0], Levels[0], whole_flags[0])
        #         paramorgtot.append(paramorg)
        #         paramcompresstot.append(paramcompress)
        #         print('ddd')
        #         count += 1

        # paramorg=[ptemp.shape[0]*ptemp.shape[1]];
        # Perm_mat, Newp = Sort_Vecs(ptemp)
        # Newp_transformed, hadDim, n_real = Hadamard_trans(Newp)
        # Chosen_row,Newp_compressed = Compress(Newp_transformed, Levels[0], whole_flags[0])
        # paramcompress = [Newp_compressed.shape[0] * Newp_compressed.shape[1]]
        # print('Compression Ratio Layer 1:', Newp_compressed.shape[0]/ptemp.shape[0])
        # Newp_hat = Hadamard_invtrans(hadDim,Chosen_row, Newp_compressed,n_real)
        # Decompressedp = (Perm_mat.T)@Newp_hat
        # # print(np.mean(np.mean(np.abs(Decompressedp-ptemp),axis=1),axis=0))
        Lay = 0
        ptemp = np.asarray(network.fc1.weight.detach().cpu())
        Decompressedp, paramorg, paramcompress = VectorizedCompress(ptemp, compressdim[Lay],Levels[Lay], whole_flags[Lay])
        # Decompressedp, paramorg, paramcompress = MatriceformCompress(ptemp, compressdim[Lay], Levels[Lay], whole_flags[Lay])
...
# #######################################
#         ptemp = np.asarray(network.fc2.weight.detach().cpu()).T
#         paramorg.append(ptemp.shape[0] * ptemp.shape[1])
#         Perm_mat, Newp = Sort_Vecs(ptemp)
#         Newp_transformed, hadDim, n_real = Hadamard_trans(Newp)
#         Chosen_row, Newp_compressed = Compress(Newp_transformed, Levels[1], whole_flags[1])
#         paramcompress.append(Newp_compressed.shape[0] * Newp_compressed.shape[1])
#         print('Compression Ratio Layer 2:', Newp_compressed.shape[0]/ptemp.shape[0])
#         Newp_hat = Hadamard_invtrans(hadDim,Chosen_row, Newp_compressed,n_real)
#         Decompressedp = (Perm_mat.T) @ Newp_hat
#         network2.fc2.weight = torch.nn.Parameter(data=torch.Tensor(Decompressedp.T), requires_grad=True)
        Lay = 1
...
###########################################
        cor = test(network2)
        # print(np.sum(np.asarray(paramorg)))
        comprate = np.sum(np.asarray(paramcompresstot))/np.sum(np.asarray(paramorgtot))
        print(comprate)
        print('dd',np.asarray(cor.cpu())/comprate)
        # H1 = hada_gen(512)
        # print(np.transpose(H1)@H1)

        # print('Model is loaded. Evaluating Model:')
        # test(network)
